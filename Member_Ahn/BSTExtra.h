...
BSTNode<T, N>* BST<T, N>::addAVLHelper(N* x, BSTNode<T, N>* t, std::function<T(N*)>* access)
{
	...
	setHeight(max(heightHelper(t->getLeft()), heightHelper(t->getRight())) + 1);
	return t;
}
...
BSTNode<T, N>* BST<T, N>::singleRightRotate(BSTNode<T, N>* &t)
{
	...
	u->setRight(t);
	t->setHeight(max(heightHelper(t->getLeft()), heightHelper(t->getRight())) + 1);
	u->setHeight(max(heightHelper(u->getLeft()), t->height()) + 1);
	return u;
}
...
{
	t->getLeft(singleLeftRotate(t->getLeft()));
...
{
	if (root) this->addAVLHelper (val, root, access);
...
{
	return this->removeAVLHelper (val, this->root, access);
}
